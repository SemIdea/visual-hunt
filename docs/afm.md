...
- **Broken windows:** build, type-check, audit ou teste quebrado vira correção imediata ou item explícito em `docs/BUGS.md`.
...
4. **Não deixar vulnerabilidade acionável sem decisão.**
   Verificação: `npm audit`. Se não corrigir, registrar em `docs/BUGS.md` com severidade e motivo.
...
4. Atualizar `docs/BUGS.md` se o risco permanecer.
...
- [ ] `npm audit` passa ou risco está em `docs/BUGS.md`.
